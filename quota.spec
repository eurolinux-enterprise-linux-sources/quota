...
# Bug #609795, included in upstream 4.00_pre1 
Patch10: quota-3.17-64-bit-quota-support.patch
# Bug #623656, included in upstram 4.00_pre1
Patch11: quota-3.17-use-proc-mounts-for-mountpoint-scanning.patch
# Bug #547748, included in upstream 4.00_pre2
Patch12: quota-3.17-Add-GFS2-support.patch
# In upstream 4.00_pre2
Patch13: quota-4.00_pre1-Add-always-resolve-to-quota.1-manpage.patch
Patch14: quota-4.00_pre1-Document-always-resolve-at-edquota-and-setquota.patch
# Bug #667755, included in upstream 4.00_pre1
Patch15: quota-3.17-Convert-quota-limits-using-rq_bsize-from-RPC-request.patch
# Bug #667755, submitted to upstream
Patch16: quota-3.17-Make-RPC-block-factor-dynamic.patch
# Bug #668709, included in upstream 4.00_pre2
Patch17: quota-3.17-Check-whether-set-limits-fit-into-the-range-supporte.patch
# Bug #668710, submitted to upstream
Patch18: quota-4.00_pre1-Check-set-limits-fit-into-the-range-supported-by-RPC.patch
# Bug #634137, submitted to upstream
Patch19: quota-4.00_pre1-Store-PID-of-quota_nld.patch
# Bug #658586, in upstream 4.00_pre2
Patch20: quota-4.00_pre1-Add-quotasync-tool.patch
# Bug #658586, in upstream 4.00_pre2
Patch21: quota-4.00_pre1-Implement-quotacheck-for-GFS2.patch
# Bug #609795, included in upstream 4.00_pre1 
Patch22: quota-4.00_pre1-fix_vfsv0_format_detection.patch
# Bug #668709, submitted to upstream
Patch23: quota-4.00_pre1-Initialize-vfsold-block-and-inode-value-boundries-fo.patch
# Bug #684017, submitted to upstream, SF#3206267
...
%patch10 -p1 -b .64bit_quota
%patch11 -p1 -b .use_proc_mounts
%patch12 -p1 -b .add_gfs2
%patch13 -p1 -b .document_always_resolve_quota
%patch14 -p1 -b .document_always_resolve_setquota
%patch15 -p1 -b .use_rq_bsize
%patch16 -p1 -b .rpc_block_factor_dynamic
%patch17 -p1 -b .check_set_limits
%patch18 -p1 -b .check_set_limits_rpc
%patch19 -p1 -b .store_pid
%patch20 -p1 -b .add_quotasync
%patch21 -p1 -b .gfs2_quotacheck
%patch22 -p1 -b .vfsv0_format_detection
...
* Fri Mar 11 2011 Petr Pisar <ppisar@redhat.com> - 1:3.17-15
- Fix ddquot buffer leak (bug #684017)

* Wed Feb 09 2011 Petr Pisar <ppisar@redhat.com> - 1:3.17-14
- Fix vfsv0 format detection (bug #609795)
- Initialize vfsold block and inode value boundries for new quota file
  (bug #668709)

* Fri Feb 04 2011 Petr Pisar <ppisar@redhat.com> - 1:3.17-13
- Correct spec file comment (bug #634137)
- Add quotasync tool (bug #658586)
- Implement quotacheck for GFS2 (bug #658586)

* Fri Feb  4 2011 Petr Pisar <ppisar@redhat.com> - 1:3.17-12
- Do not own common directories under /etc (bug #634137)
- Store quota_nld PID into PID file (bug #634137)
- Do not allow non-root to control quota_nld service (bug #634137)

* Fri Jan 14 2011 Petr Pisar <ppisar@redhat.com> - 1:3.17-11
- Add support for 64-bit vfsv1 quota format (bugs #609795, #669598)
- Use /proc/mounts for mountpoint scanning (#623656)
- Add quota_nld deamon init script (bug #634137)
- Comment example quotatab to silent warnquota (bug #663027)
- Add GFS2 query support via RPC (bug #547748)
- Document --always-resolve option (bug #662997)
- Use rq_bsize to convert quotas transferred by RPC (bug #667755)
- Make RPC block factor dynamic (bug #667755)
- Check whether set limits fit into the range supported by quota format
  (bug #668709)
- Check set limits fit into the range supported by RPC transport (bug #668710)
